...
copying it to `glazier/.git/hooks/pre-push`:
...
echo "cargo fmt"
cargo fmt --all -- --check
echo "cargo clippy glazier"
cargo clippy --manifest-path=glazier/Cargo.toml --all-targets -- -D warnings
echo "cargo test glazier"
cargo test --manifest-path=glazier/Cargo.toml
```
...
The `Cargo.toml` file needs to be updated.  The `version` field needs to be increased to the 
next [semver] version that makes sense. These packages all also import each other and those 
cross-dependency versions need updating too.
...
```sh
cargo publish --manifest-path="glazier/Cargo.toml"
```
